...
export async function getManagedUsers(): Promise<User[]> {
  ...
  return (data || []).map((record: Record<string, unknown>) => {
    const typedRecord: RawUserRecord = {
      ...record,
      role: record.role as RawUserRecord['role'],
    } as RawUserRecord;
    return mapUser(typedRecord);
  });
}
...
export async function getEmployeesWithoutUserAccess(): Promise<EmployeeWithoutAccess[]> {
  ...
  return (allEmployees || [])
    .filter((emp: RawEmployeeRecord): emp is RawEmployeeRecord => !employeesWithAccess.has(emp.id))
    .map(mapEmployee);
}
